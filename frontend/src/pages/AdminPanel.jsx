        ...
        <div className="admin-panel-container" style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div className="admin-tabs" style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
                ...
                {user?.can_manage_products && (
                    <button onClick={() => setActiveTab('products')} style={tabStyle(activeTab === 'products')}>Productos</button>
                )}
                {user?.can_manage_promotions && (
                    <button onClick={() => setActiveTab('promotions')} style={tabStyle(activeTab === 'promotions')}>Promociones</button>
                )}
                <button onClick={() => setActiveTab('emails')} style={tabStyle(activeTab === 'emails')}>Correos</button>
                <button onClick={() => setActiveTab('bot')} style={tabStyle(activeTab === 'bot')}>Bot WhatsApp</button>
            </div>

            <div className="tab-content" style={{ border: '1px solid #ddd', padding: '20px', borderRadius: '8px' }}>
                {activeTab === 'welcome' && <p>Selecciona una opción del menú.</p>}
